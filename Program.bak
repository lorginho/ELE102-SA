...
namespace LetterU3DApp
{
    public class Game : GameWindow
    {
        ...
        private bool showGrid = false; // Desactivar cuadrículas por defecto
        ...
        protected override void OnRenderFrame(FrameEventArgs e)
        {
            ...
            // Dibujar ejes y cuadrículas si están habilitados
            if (showAxes)
                DrawAxes();
            if (showGrid)
                DrawGrid();
            ...
            Title = $"Letra U Wireframe - Letra Activa: {activeLetterIndex + 1} | " +
                    ...
                    $"Cuadrícula: {(showGrid ? "Visible" : "Oculta")}";
        ...
        private void DrawAxes()
        {
            ...
            DrawText(new Vector3(axisLength, 0, 0), "X", Color4.Red);
            DrawText(new Vector3(0, axisLength, 0), "Y", Color4.Green);
            DrawText(new Vector3(0, 0, axisLength), "Z", Color4.Blue);
        }
        ...
        private void DrawGrid()
        {
            ...
            {
                float pos = i * (gridSize / gridLines);

                GL.Vertex3(pos, -gridSize, 0);
                GL.Vertex3(pos, gridSize, 0);

                GL.Vertex3(-gridSize, pos, 0);
                GL.Vertex3(gridSize, pos, 0);
            }
            ...
            for (int i = -gridLines; i <= gridLines; i++)
            {
                ...
                GL.Vertex3(pos, 0, -gridSize);
                GL.Vertex3(pos, 0, gridSize);

                GL.Vertex3(-gridSize, 0, pos);
                GL.Vertex3(gridSize, 0, pos);
            }
            ...
            for (int i = -gridLines; i <= gridLines; i++)
            {
                ...
                GL.Vertex3(0, pos, -gridSize);
                GL.Vertex3(0, pos, gridSize);

                GL.Vertex3(0, -gridSize, pos);
                GL.Vertex3(0, gridSize, pos);
            }
        ...
        private void DrawText(Vector3 position, string text, Color4 color)
        {
            // Implementación simplificada para dibujar texto en OpenGL
            // Aquí puedes usar una biblioteca como FreeType para dibujar texto si lo deseas
            // Por ahora, esto es solo un marcador de posición para mostrar la idea
            ...
            {
                GL.CallLists(1, ListNameType.UnsignedByte, new byte[] { (byte)c });
            }
        }
        ...
        protected override void OnUpdateFrame(FrameEventArgs e)
        {
            ...
            // Mostrar/ocultar cuadrícula al presionar F6
            if (keyState.IsKeyDown(Key.F6) && !previousKeyState.IsKeyDown(Key.F6))
            {
                showGrid = !showGrid;
            }
